<#
.SYNOPSIS
    Removes users from a list from multiple specified Active Directory groups across multiple domains.

.DESCRIPTION
    This script reads usernames from a text file and group information from a CSV file, then removes 
    users from the specified groups. The script searches across all domains in the forest to find 
    users and handles cases where users or groups are not found. All inputs are collected via 
    interactive prompts.

.EXAMPLE
    .\Remove_Members-from-Cross_Domain-Groups.ps1
...
.NOTES
    ...
    5. Run the script: .\Remove_Members-from-Cross_Domain-Groups.ps1
...
Write-Host "This script will remove users from a list from multiple Active Directory groups" -ForegroundColor White
...
$confirmation = Read-Host "Proceed with REMOVAL from these groups? (Y/N)"
...
try {
    ...
    foreach ($UserName in $UserNames) {
        ...
        # Remove each found user from each verified group
        foreach ($FoundUser in $FoundUserObjects) {
            foreach ($TargetGroup in $VerifiedGroups) {
                try {
                    Remove-ADGroupMember -Identity $TargetGroup.GroupObject -Members $FoundUser.UserObject -Server $TargetGroup.Domain -Confirm:$false -ErrorAction Stop
                    ...
                    Write-Host "    ✓ Removed from $($TargetGroup.GroupName) in $($TargetGroup.Domain)" -ForegroundColor Green
                } catch {
                    ...
                    Write-Warning "    ✗ Failed to remove from $($TargetGroup.GroupName): $($_.Exception.Message)"
                }
            }
        }
    }
